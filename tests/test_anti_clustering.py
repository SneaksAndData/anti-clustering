from anti_clustering import SimulatedAnnealingHeuristicAntiClustering, ExactClusterEditingAntiClustering, \
    ExchangeHeuristicAntiClustering, NaiveRandomHeuristicAntiClustering
import pytest
import pandas as pd
...
def test_verbose_mode(algorithm):
    ...
    algorithm.run(df=df, numerical_columns=['x', 'y'], num_groups=2, destination_column='col', categorical_columns=['c'])
